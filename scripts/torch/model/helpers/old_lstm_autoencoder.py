            ...
            for ENCODING_DIM in LIST_ENCODING_DIM:
                for SEQL in LIST_SEQL:
                    ...
                    if MODEL_ACTION == 'LOAD':  # cargamos datos anómalos para testear
                        ...
                        data_x_seizure = data_x_seizure[np.unique(np.where(data_y_seizure == 1)[0]).tolist()]  # type: ignore
                        data_y_seizure = data_y_seizure[np.unique(np.where(data_y_seizure == 1)[0]).tolist()]  # type: ignore
                    ...
                    for idx, (train_index, test_index) in enumerate(kf.split(data_x_normal)):
                        ...
                        MODEL_NAME = 'kf_'+str(idx)+'_tc_'+str(CONF_CHANNELS)+'_ep_'+str(N_EPOCHS)+'_sL_'+str(SEQL)+'_eD_'+str(
                            ENCODING_DIM)+'_lr_'+str(LEARNING_RATE)+'_T_'+str(len(train_normal))+'_t_'+str(len(test_normal))
                        ...
                        if MODEL_ACTION == 'TRAIN':
                            model, history = model.train_model(model, train_normal, test_normal, train_index, test_index,
                                                               N_EPOCHS, LEARNING_RATE, MODEL_NAME, PLOT_RESULT)
                            print('TRAIN Completed!!\n')

                        elif MODEL_ACTION == 'LOAD' and test_seizure is not None:  # type: ignore
                            print(f'Loading model...', end=" ")
                            model.load_state_dict(torch.load(str(MODEL_NAME+'.pth'), map_location=model.device))  # type: ignore
                            print('DONE \n')

                            # load losses dict
                            with open(str(MODEL_NAME+'.pickle'), 'rb') as handle:  # type: ignore
                            ...
                            test_seizure = test_seizure[:len(test_normal)]  # type: ignore
